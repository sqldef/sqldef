...
func parseOptions(args []string) (database.Config, *sqldef.Options) {
	// Track parsed configs in order, starting with PostgreSQL-specific default
	configs := []database.GeneratorConfig{{LegacyIgnoreQuotes: true}}
	...
	opts.Config = func(path string) {
		configs = append(configs, database.ParseGeneratorConfig(path, true))
	}
	opts.ConfigInline = func(yaml string) {
		configs = append(configs, database.ParseGeneratorConfigString(yaml, true))
	}
	...
	if opts.Help {
		...
		fmt.Printf("\nFor more information, see: https://github.com/sqldef/sqldef/blob/%s/cmd-psqldef.md\n", gitRef)
		os.Exit(0)
	}
