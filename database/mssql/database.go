...
func buildExportTableDDL(table string, columns []column, indexDefs []*indexDef, foreignDefs []string, checkDefs []string) string {
	...
	for i, col := range columns {
		...
		if col.DefaultVal != "" {
			// Always include constraint name if present, even if auto-generated
			// This is necessary for DROP operations to work correctly
			if col.DefaultName != "" {
				fmt.Fprintf(&queryBuilder, " CONSTRAINT %s DEFAULT %s", quoteName(col.DefaultName), col.DefaultVal)
			} else {
				fmt.Fprintf(&queryBuilder, " DEFAULT %s", col.DefaultVal)
			}
		}
		if col.Identity != nil {
...
type column struct {
	...
	DefaultName              string
	DefaultVal               string
	DefaultNameAutoGenerated bool // true if DefaultName was auto-generated by SQL Server
	Check                    *check
}
...
`
	...
	for rows.Next() {
		...
		col.Scale = scale
		if defaultId != "0" {
			// Keep the constraint name even if auto-generated (needed for DROP operations)
			col.DefaultName = *defaultName
			col.DefaultNameAutoGenerated = isAutoGeneratedConstraintName(*defaultName)
			// SQL Server wraps default values in parentheses - normalize by removing outer parens
			col.DefaultVal = normalizeDefaultValue(*defaultVal)
		}
...
// isAutoGeneratedConstraintName checks if a constraint name was auto-generated by SQL Server
// Auto-generated default constraint names follow the pattern: DF__tablename__columnname__hash
func isAutoGeneratedConstraintName(name string) bool {
	return strings.HasPrefix(name, "DF__")
}

// normalizeDefaultValue removes all outer parentheses from SQL Server default values
// SQL Server stores defaults like ((1)) or (getdate()), we normalize to 1 or getdate()
// This allows comparison with desired schemas that don't have parentheses
func normalizeDefaultValue(val string) string {
