...
func (g *Generator) normalizeViewDefinition(definition string) string {
	// Remove SQL comments before any other processing
	// Remove -- style comments
	// Parser output may have comments like "-- pattern 1" inline without newlines
	// Match -- followed by typical comment patterns (words/numbers) and trailing space
	definition = regexp.MustCompile(`--\s*[a-zA-Z]+\s+\d+\s+`).ReplaceAllString(definition, "")
	// Also match -- followed by content until newline (for cases with newlines)
	definition = regexp.MustCompile(`--[^\n]*\n`).ReplaceAllString(definition, " ")
	// Remove /* */ style comments
	...
	if g.mode == GeneratorModePostgres {
		...
		// Remove type casts from string and numeric literals
		// PostgreSQL adds these when exporting views: 'text'::text, 'text'::character varying, 123::integer
		// Match literal::type patterns and remove the ::type part
		// This handles: 'string'::text, 'str'::character varying, 'str'::varchar, 'str'::bpchar
		definition = regexp.MustCompile(`'([^']*)'::(?:text|character varying|varchar|bpchar)`).ReplaceAllString(definition, "'$1'")
		// Also handle numeric literals: 123::integer, 123::bigint, 123::numeric
		definition = regexp.MustCompile(`(\d+(?:\.\d+)?)::(?:integer|bigint|numeric|smallint|real|double precision)`).ReplaceAllString(definition, "$1")
		// Handle array literals: '{}'::int[], ARRAY[]::text[]
		definition = regexp.MustCompile(`(?:'{}' |array\[\])::(\w+)\[\]`).ReplaceAllString(definition, "array[]")

		// PostgreSQL transforms jsonb_extract_path_text calls to use VARIADIC ARRAY syntax
		// Normalize: jsonb_extract_path_text(x, VARIADIC ARRAY['a'::text, 'b'::text]) -> jsonb_extract_path_text(x, 'a', 'b')
		definition = regexp.MustCompile(`variadic array\[([^\]]+)\]`).ReplaceAllString(definition, "$1")
		// Also remove standalone "variadic" keyword
		definition = regexp.MustCompile(`\bvariadic\s+`).ReplaceAllString(definition, "")

		// PostgreSQL adds ELSE NULL to CASE expressions
		// Remove: ELSE NULL::text, ELSE NULL::integer, etc.
		definition = regexp.MustCompile(`\s+else\s+null(?:::\w+)?`).ReplaceAllString(definition, "")

		// Normalize CAST syntax: convert CAST(x AS type) to x::type for consistent comparison
		// This handles parser output which uses CAST() while PostgreSQL uses ::
		definition = regexp.MustCompile(`cast\(([^)]+)\s+as\s+(\w+)\)`).ReplaceAllString(definition, "($1)::$2")

		// PostgreSQL adds intermediate type casts in function arguments
		// Handle nested patterns like ((x)::bigint)::double precision
		// First, remove outermost double precision/real casts
		definition = regexp.MustCompile(`::(double precision|real)`).ReplaceAllString(definition, "")
		// Then remove extra nested parentheses around casts: ((x))::type -> (x)::type
		for i := 0; i < 3; i++ { // Run multiple times to handle multiple levels of nesting
			definition = regexp.MustCompile(`\(\(([^()]+)\)\)::(\w+)`).ReplaceAllString(definition, "($1)::$2")
		}
		...
		// Remove unnecessary parentheses in WHERE clauses
		// PostgreSQL adds parentheses around WHERE conditions: "WHERE (bar = 'baz')" -> "WHERE bar = 'baz'"
		// Match WHERE followed by an opening paren, then content until a closing paren
		// Need to be careful not to remove functional parentheses in expressions
		definition = regexp.MustCompile(`\bwhere\s+\(([^()]+)\)`).ReplaceAllString(definition, "where $1")
		...
		// Normalize parentheses spacing: remove spaces after ( and before )
		definition = regexp.MustCompile(`\(\s+`).ReplaceAllString(definition, "(")
		definition = regexp.MustCompile(`\s+\)`).ReplaceAllString(definition, ")")

		// Remove unnecessary outer parentheses around expressions before AS
		// Need to handle nested parentheses, so run multiple times
		// This handles patterns like (function(...)) as, (cast(...) as type) as, etc.
		for i := 0; i < 5; i++ {
			// Try to remove outer parens around balanced expressions before 'as'
			// This is a simplified approach that works for common cases
			before := definition
			// Match opening paren, then balanced content, then closing paren, then ' as'
			// We'll use a different strategy: remove parens when they wrap the entire expression before 'as'
			definition = regexp.MustCompile(`,\s*\(([^,]+)\)\s+as\b`).ReplaceAllString(definition, ", $1 as")
...
func (g *Generator) normalizeIndexColumn(column string) string {
	...
	if g.mode == GeneratorModePostgres {
		...
		// Remove unnecessary parentheses around simple boolean expressions
		// PostgreSQL adds these: (is_active IS TRUE) -> is_active IS TRUE
		column = regexp.MustCompile(`\((\w+\s+is\s+(?:true|false|null))\)`).ReplaceAllString(column, "$1")

		// Normalize parentheses spacing
		column = regexp.MustCompile(`\(\s+`).ReplaceAllString(column, "(")
		column = regexp.MustCompile(`\s+\)`).ReplaceAllString(column, ")")
